...
def signup(request):
    if request.method == 'POST':
        ...
        if password1 == password2 and len(Account.objects.filter(email=email)) == 0:
            ...
            send_mail('Verify Your Account', message, 'learnhere.21@gmail.com', [email, ], fail_silently=True)
...
def Reg(request,mode,otp,dtk):
        ...
        if mode == 'student':
            if int(STUDENT_APPLICATION[dtk][0]) == int(otp):
                ...
                send_mail(subject,message,'learnhere.21@gmail.com',[STUDENT_APPLICATION[dtk][3], ], fail_silently=True)
